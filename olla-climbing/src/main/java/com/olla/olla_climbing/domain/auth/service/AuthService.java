...
//클라이언트(앱): 스마트폰 기기 내부(안전한 저장소)에 보관하고 있던 Access Token과 Refresh Token을 깨끗하게 지워서 비워버립니다.
//서버(백엔드): 클라이언트가 지웠다고 해도, 혹시나 나쁜 놈이 토큰을 복사해 뒀을 수 있으니 **DB(refresh_token 테이블)에 저장된 해당 유저의 토큰 기록을 아예 삭제(Delete)**해버립니다.